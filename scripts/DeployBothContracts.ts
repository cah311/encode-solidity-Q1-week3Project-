...
async function main() {
  ...
  console.log(
    `The MyToken contract was deployed at address ${myTokenContract.address} in the block ${txTokenReceipt.blockNumber}`
  );
  ...
  const tokenizedBallotContract = await tokenizedballotFactory.deploy(
    ...
    txTokenReceipt.blockNumber
  );
  console.log("Tokenized Ballot Arguments:");
  console.log(
    `Proposal Bytes: ${proposals.map((p) =>
      ethers.utils.formatBytes32String(p)
    )}`
  );
  console.log(`MyToken Contract: ${myTokenContract.address}`);
  console.log(`BlockNUmber: ${txTokenReceipt.blockNumber}`);
  console.log("awaiting for confirmation...");
  const txReceipt = await tokenizedBallotContract.deployTransaction.wait();
  console.log(
    `The tokenized ballot contract was deployed at address ${tokenizedBallotContract.address} in the block ${txReceipt.blockNumber}`
  );
}
